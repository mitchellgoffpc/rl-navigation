...
def train():
  ...
  value_model.load_state_dict(torch.load(Path(__file__).parent / 'checkpoints/value.ckpt'))
  ...
  for _ in range(5):
    ...
    for i in trange(0, len(train_steps), BATCH_SIZE, leave=False, desc="VALUE POLICY - TRAIN"):
      states, goals, _, _, _, correct_actions, *_ = batch(train_steps[i:i+BATCH_SIZE])
      with torch.no_grad():
        # distances = value_model(states, goals)
    ...
    policy_model.eval()
    with torch.no_grad():
      for i in trange(0, len(val_steps), BATCH_SIZE, leave=False, desc="VALUE POLICY - VAL"):
        states, goals, _, _, _, correct_actions, *_ = batch(train_steps[i:i+BATCH_SIZE])
        with torch.no_grad():
          distances = value_model(states, goals)
          targets = distances.argmin(dim=-1)
    ...
    # print metrics
    torch.save(policy_model.state_dict(), Path(__file__).parent / 'checkpoints/policy-vb.ckpt')
    print(f"VALUE POLICY: "
